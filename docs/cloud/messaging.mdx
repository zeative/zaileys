...
```mermaid
flowchart TD
  closed["<b>Window closed</b><br/>Templates only — free-form sends<br/>fail with Meta code 131047"]
  open["<b>Window open</b><br/>Any message works: text, media,<br/>buttons, lists, templates"]
  closed -->|"customer messages you"| open
  open -->|"customer messages again:<br/>the 24 hours restart"| open
  open -->|"24 hours with no<br/>customer message"| closed
  closed -->|"you send a template<br/>(the window stays closed)"| closed
```
...
## When something isn't supported {#when-something-isnt-supported}
