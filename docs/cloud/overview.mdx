...
sequenceDiagram
  ...
  User->>WA: Sends "hi"
  WA->>Meta: Delivers the message
  Meta->>Hook: POST, signed with your app secret
  Hook->>Hook: client.webhook() checks the signature
  Hook->>Bot: Emits the text event
  Hook-->>Meta: 200 OK
  Bot->>Meta: msg.reply() calls the Graph API
...
| **What you can send** | Every message type | Text, media, locations, contacts, reactions, up to 3 reply buttons, and lists. Anything else rejects with `NOT_IMPLEMENTED`. [Messaging](/cloud/messaging) |
