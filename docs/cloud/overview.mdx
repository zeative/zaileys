...
- `message-status` fires on both providers. After a reply, it reports `sent`, then `delivered`, then `read`, as the customer's phone reports each step. On the Cloud API it also carries `conversationId` and `error`, which a linked device has no equivalent for.
...
| **Delivery and read receipts** | The `message-status` event, decoded from acks | The same `message-status` event, from Meta's status webhooks. [Cloud events](/cloud/events) |
