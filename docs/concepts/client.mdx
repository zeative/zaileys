...
```mermaid
stateDiagram-v2
  state "qr-pending" as qr_pending
  state "pairing-pending" as pairing_pending

  [*] --> idle
  idle --> connecting: connect()
  connecting --> connected: saved session found
  connecting --> qr_pending: no session, QR login
  connecting --> pairing_pending: no session, pairing login
  qr_pending --> connected: QR scanned
  pairing_pending --> connected: code entered
  connecting --> reconnecting: connection dropped
  connected --> reconnecting: connection dropped
  reconnecting --> connecting: after a short wait
  connected --> disconnected: logged out from the phone
  connected --> disconnecting: disconnect() or logout()
