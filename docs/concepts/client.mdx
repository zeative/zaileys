...
### Disconnects that stop the client

Three reasons stop the client instead of retrying. Only a logout deletes the saved login — the other two keep it, so the next start connects without a new QR code:

| Reason | What happened | Retried? | Saved login |
| --- | --- | --- | --- |
| `logged-out` | The device was removed in **WhatsApp → Linked devices**, or you called `logout()`. | No | Deleted, after a backup |
| `connection-replaced` | Another connection using the same session took over. | No | Kept |
| `forbidden` | WhatsApp refused the account. | No | Kept |

WhatsApp sometimes reports `logged-out` by mistake right after a successful connection, so the client retries that case up to two times, 3 seconds apart, before it deletes anything. Every other reason is retried with the saved login, including `bad-session`. To delete the login on more reasons, see [When the saved session is deleted](/whatsapp-web#when-the-saved-session-is-deleted). The [events reference](/reference/events) lists them all.
