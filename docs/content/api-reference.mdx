...
| `groupStatus(source, opts?)` | `(source: GroupStatusSource, opts?: GroupStatusRepostOptions)` | Repost an existing message as a group status — copies media pointers, no re-upload. |
