...
| `groupStatus(text, opts?)` | `(text: string, opts?: GroupStatusOptions)` | Group status post (`backgroundColor`, `font`, experimental `audience`). Groups only. |
| `groupStatus(source, opts?)` | `(source: GroupStatusSource, opts?: GroupStatusRepostOptions)` | Repost a message, or post fresh media (`{ image\|video\|audio, caption?, ptt?, audience? }`), as a group status (audience metadata is experimental). Media is always re-uploaded. |
