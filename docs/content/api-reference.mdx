...
| `groupStatus(source, opts?)` | `(source: GroupStatusSource, opts?: GroupStatusRepostOptions)` | Repost an existing **text** message as a group status. Media sources reject with `INVALID_OPTIONS` (never renders inside the envelope). |
