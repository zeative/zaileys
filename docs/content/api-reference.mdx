...
| `groupStatus(source, opts?)` | `(source: GroupStatusSource, opts?: GroupStatusRepostOptions)` | Repost a message, or post fresh media (`{ image\|video\|audio, caption?, ptt? }`), as a group status. |
