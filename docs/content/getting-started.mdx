---
description: "Build your first WhatsApp bot with Zaileys in minutes — connect via QR or pairing code, listen for messages, and reply. A step-by-step guide with examples."
---
...
<Tabs items={['bun', 'tsx (Node)', 'ts-node']}>
  <Tabs.Tab>
```bash
bun run bot.ts
```
