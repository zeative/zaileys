---
title: "The WhatsApp Framework for Node.js & TypeScript — Unofficial + Official Cloud API"
description: "Zaileys is a type-safe, batteries-included WhatsApp framework for Node.js and TypeScript. One API, two providers: the unofficial WhatsApp Web engine (Baileys) and the official Meta Cloud API — same Client, same builder, same typed events."
---
...
<Cards>
  <Cards.Card icon="🔗" title="Unofficial · WhatsApp Web" href="/unofficial" arrow>
    Powered by [Baileys](https://github.com/WhiskeySockets/Baileys). Log in with a QR or pairing
    code, no Meta approval. Full personal-account power: groups, communities, channels, polls,
    stickers, presence, edit/delete. **Default provider.**
  </Cards.Card>
  <Cards.Card icon="☁️" title="Official · Meta Cloud API" href="/cloud-api" arrow>
    The sanctioned Graph API + webhooks. No ban risk, no device linking. Templates, OTP, marketing
    campaigns, Flows, catalog commerce, analytics — everything a business channel needs.
  </Cards.Card>
</Cards>
...
<Tabs items={['Unofficial (WhatsApp Web)', 'Official (Meta Cloud API)']}>
<Tabs.Tab>
```typescript
...
webhook wiring differ. See the [Official Cloud API](/cloud-api) guide.
</Tabs.Tab>
</Tabs>
...
| Run a compliant business channel with no ban risk | **[Official](/cloud-api)** (Meta Cloud API) |
| Send OTP, marketing, or utility **template** messages at scale | **[Official](/cloud-api)** |
| Message users who never texted you first (business-initiated) | **[Official](/cloud-api)** — via approved templates |
...
| Templates & campaigns    | ☁️ | Template CRUD, OTP/marketing/utility sends, `message-status` tracking | [Official Cloud API](/cloud-api)       |
| Flows & commerce         | ☁️ | WhatsApp Flows, catalog products, orders, analytics                  | [Official Cloud API](/cloud-api)       |
