...
<Cards>
  <Cards.Card icon="🔗" title="Unofficial · WhatsApp Web" href="/unofficial" arrow>
    The default. Baileys under the hood. QR / pairing login, personal-account superpowers.
  </Cards.Card>
  <Cards.Card icon="☁️" title="Official · Meta Cloud API" href="/cloud-api" arrow>
    Graph API + webhooks. Token auth, business-grade, no ban risk, templates & campaigns.
  </Cards.Card>
</Cards>
...
| Rich responses (AIRich markdown) | ✅ | ✅ (rendered as text) |
| Edit / Delete / Pin | ✅ | ❌ |
...
See [Official Cloud API → The 24-hour window](/cloud-api#the-24-hour-window) for the full flow.
...
2. **Add a webhook** — the official provider is push-based; expose `client.webhook()` on a public
   URL. See [Official Cloud API → Webhook](/cloud-api#webhook).
...
<Cards>
  ...
  <Cards.Card icon="☁️" title="Official Cloud API guide" href="/cloud-api" arrow />
