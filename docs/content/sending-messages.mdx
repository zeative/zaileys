...
<Callout type="warning">
  **Media status may not render on every account.** Text group status is verified working. Media
  relays cleanly and returns a message id, but on the account used for development it never appeared,
  while an identical proto captured from another sender *did* render for them — same structure, same
  library, same code, even a raw baileys socket. Everything under our control was eliminated, so the
  remaining variable appears to be account or client rollout. Test it on your own account before
  relying on it.
</Callout>
