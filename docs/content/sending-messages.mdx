...
<Callout type="warning">
  **Text only.** Reposting an image, video, or voice note rejects with `INVALID_OPTIONS`. This is a
  WhatsApp limitation, not a missing feature: live testing on 2026-08-12 confirmed the server *accepts*
  a media group status and returns a message id, but no client ever renders it. Baileys' `getMediaType`
  does not look inside the `groupStatusMessageV2` envelope, so the stanza ships without its `mediatype`
  attribute — and that attribute is set on the inner `enc` node, which callers cannot reach. Rather than
  let you send something that silently disappears, zaileys rejects it up front.
</Callout>
