...
Pass a message instead of text and it becomes the status. Media is carried by **copying its pointers**,
so nothing is downloaded and nothing is re-uploaded — reposting a 100 MB video costs the same as
reposting a line of text.
...
Accepts text, image, video, and voice notes. `viewOnce` is stripped and any inherited quote chain is
cleared, so the status does not carry the original's context.

| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `caption` | `string` | original caption | Replaces the caption. Ignored for voice notes. |

<Callout type="warning">
  Because only the media pointers are copied, a reposted status breaks if WhatsApp expires the original
  media — the same trade-off forwarding has.
</Callout>

<Callout type="error">
  A non-group recipient rejects with `INVALID_RECIPIENT`. Empty text, or a source with no repostable
  content, rejects with `EMPTY_CONTENT`. A malformed colour, an unknown font, or an unsupported source
  type (sticker, document, poll, location, contact) rejects with `INVALID_OPTIONS`.
</Callout>
