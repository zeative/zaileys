...
Pass a message instead of text and its text becomes the status. Any inherited quote chain is cleared,
so the status does not carry the original's context.
...
<Callout type="warning">
  **Text only.** Reposting an image, video, or voice note rejects with `INVALID_OPTIONS`. Eight
  variants were relayed live — plain, with `contextInfo.isGroupStatus`, with `statusSourceType`, with
  `statusAttributions`, with `statusJidList` — and none rendered, while the same session posted a media
  `status@broadcast` that *did* render. So the limit is the group-status envelope itself, not the media
  pipeline. Text in that same envelope renders fine. A media status that silently vanishes is worse
  than a clear error, so zaileys blocks it.
</Callout>
