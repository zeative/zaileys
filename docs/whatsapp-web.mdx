...
flowchart LR
  ...
  fatal -->|yes| stop["Delete the session<br/>and stop"]
...
| --- | --- | --- | --- |
| `logged-out` | 401 | The device was removed from the phone, or logged out. | Deletes the session and stops. Right after connecting it first retries up to twice, because WhatsApp sometimes sends a false logout. |
| `connection-replaced` | 440 | Another connection opened with the same session. | Deletes the session and stops. |
| `forbidden` | 403 | WhatsApp refused access to the account. | Deletes the session and stops. |
| `bad-session` | 500 | The saved session can't be used. | Deletes the session and reconnects, so you get a new QR code. |
