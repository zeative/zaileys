...
| `groupStatus(text, opts?)` | `(text: string, opts?: GroupStatusOptions)` | Group status post (`backgroundColor`, `font`). Groups only. |
| `groupStatus(source, opts?)` | `(source: GroupStatusSource, opts?: GroupStatusRepostOptions)` | Repost a message, or post fresh media (`{ image\|video\|audio, caption?, ptt? }`), as a group status. Media is always re-uploaded. |
...
- Content-derived flags: `isEdited`, `isDeleted`, `isPinned`, `isUnPinned`, `isBot`, `isStatusMention`, `isGroupStatusMention`, `isGroupStatus`, `isStory`, `isHideTags`.
