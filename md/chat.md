...
| `fetchHistory` | `fetchHistory(count, oldest, oldestTimestamp)` | `Promise<string>` | Ask WhatsApp to resend older messages. Returns a request id. |
...
## `fetchHistory(count, oldest, oldestTimestamp)`

```typescript
fetchHistory(count: number, oldest: WAMessageKey, oldestTimestamp: number): Promise<string>
```

Asks WhatsApp to resend messages older than the one you pass. Useful for backfilling a chat your bot
joined late.

| Parameter | Type | Description |
| --------- | ---- | ----------- |
| `count` | `number` | How many older messages to request. |
| `oldest` | `WAMessageKey` | Key of the oldest message you already have. Must carry a `remoteJid`. |
| `oldestTimestamp` | `number` | That message's timestamp, in seconds. |

```typescript
const [oldest] = await client.store.listMessages(jid, { limit: 1 })
if (oldest) {
  await client.chat.fetchHistory(50, oldest.key, oldest.messageTimestamp)
}
```

**The messages do not come back from this call.** It returns a request id; the messages themselves
arrive later on the [`history-sync`](/events) event. Listen for that event rather than awaiting a
result here.

A key without a `remoteJid` rejects with `OPERATION_FAILED` — WhatsApp cannot tell which chat to
backfill.
