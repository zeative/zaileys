...
| `lidToPns` | `lidToPns(lids: string[]): Promise<Map<string, string>>` | Bulk `lidToPn`. Returns a `lid → pn` map. |
| `pnToLids` | `pnToLids(pns: string[]): Promise<Map<string, string>>` | Bulk `pnToLid`. Returns a `pn → lid` map. |
...
Resolving a whole participant list one jid at a time is N round trips. The bulk forms do it in one:

```typescript
const meta = await client.group.metadata('120363xxx@g.us')
const map = await client.lidToPns(meta.participants.map((p) => p.id))

for (const p of meta.participants) {
  console.log(p.id, '→', map.get(p.id) ?? 'unknown')
}
```

Jids WhatsApp could not resolve are simply **absent** from the map rather than mapped to `null`, so
`map.size` may be smaller than the input. Like the single-jid forms, these never throw — a failed
