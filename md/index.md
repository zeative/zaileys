# The WhatsApp Framework for Node.js & TypeScript — Unofficial + Official Cloud API

> Source: https://zeative.github.io/zaileys

# Zaileys

**One framework. Two WhatsApp APIs. Zero rewrites.**

Zaileys is a type-safe, batteries-included WhatsApp framework for Node.js and TypeScript. Write your
bot **once** against a single chainable builder and fully-typed events — then run it on either
provider by flipping one option:

<ProviderCards />

The `send(jid).text()` builder, the `on('text' | 'image' | 'reaction' | …)` events, commands,
middleware, plugins, storage, `broadcast()` and `scheduleAt()` — **all identical across both
providers.** New to Zaileys? Start with **[Choose Your Provider](/providers)**.

## Hello world

The exact same bot logic — only the `Client` config differs.

```typescript

// Default provider — logs in with a QR you scan from your phone.
...
Scan the printed QR from **WhatsApp → Linked Devices** and every text gets a reply. No boilerplate,
no manual decoding, no `any`.

```typescript

// Official provider — a permanent token authenticates you. No QR.
const client = new Client({
  provider: 'cloud',
  cloud: {
...
// Inbound arrives via webhook — mount on any server that speaks Web Request/Response.
export const GET = client.webhook()
export const POST = client.webhook()
```

The event handlers are **byte-for-byte the same** as the unofficial tab. Only construction and the
webhook wiring differ. See the [Official Cloud API](/official) guide.

By default the `Client` connects on construction (`autoConnect: true`), so handlers registered
synchronously after `new Client()` are wired up before the first event arrives. Opt out with
`new Client({ autoConnect: false })` and call `await client.connect()` yourself. See
[Configuration](/configuration) and the [Client](/client) reference.

Prefer a pairing code over scanning a QR on the unofficial provider? Provide your number:
...
| If you want to… | Use |
| --- | --- |
| Build a personal-account bot, prototype, or automate your own number fast | **[Unofficial](/unofficial)** (WhatsApp Web) |
| Use groups, communities, channels, polls, presence, edit/delete | **[Unofficial](/unofficial)** — these don't exist on the Cloud API |
| Run a compliant business channel with no ban risk | **[Official](/official)** (Meta Cloud API) |
| Send OTP, marketing, or utility **template** messages at scale | **[Official](/official)** |
| Message users who never texted you first (business-initiated) | **[Official](/official)** — via approved templates |

Full side-by-side breakdown, ToS notes, and a migration guide on **[Choose Your Provider](/providers)**.

## Why Zaileys

- **Two providers, one codebase** — the unofficial WhatsApp Web engine and the official Meta Cloud
  API behind the same `Client`. Switch with a single option; your handlers never change. See
...
Legend — 🔗 Unofficial (WhatsApp Web) · ☁️ Official (Meta Cloud API) · 🔗☁️ both.

| Capability               | Providers | What you get                                                          | Guide                                  |
| ------------------------ | :-------: | -------------------------------------------------------------------- | -------------------------------------- |
| Connection lifecycle     | 🔗☁️ | Auto-connect, QR + pairing-code login (unofficial) or token auth (official), reconnect, clean logout | [Client](/client) · [Config](/configuration) |
| Typed events             | 🔗☁️ | `text`, `image`, `reaction`, `button-click`, `group-update`, and more | [Events](/events)                      |
| Send builder             | 🔗☁️ | Chainable `text` / `image` / `video` / `audio` / `document` / `poll` / `album` · `reply` · `mentions` | [Sending Messages](/sending-messages)  |
| Media                    | 🔗☁️ | Lazy image/video/audio/sticker processing, optional `sharp` accelerator | [Media](/media)                        |
| Interactive UI           | 🔗☁️ | Buttons (reply/url/copy/call), lists, carousels                       | [Interactive](/interactive)            |
| Rich responses           | 🔗 | Markdown → Meta-AI-style messages (code, tables, images, directives)  | [Rich Responses](/rich-responses)      |
| Command framework        | 🔗☁️ | `client.command()` routers, aliases, configurable prefixes, middleware | [Commands](/commands)                  |
| Automation               | 🔗☁️ | Rate-limited `broadcast()`, `scheduleAt()`, presence control          | [Automation](/automation)              |
| Templates & campaigns    | ☁️ | Template CRUD, OTP/marketing/utility sends, `message-status` tracking | [Official Cloud API](/official)       |
| Flows & commerce         | ☁️ | WhatsApp Flows, catalog products, orders, analytics                  | [Official Cloud API](/official)       |
| Groups & channels        | 🔗 | Groups, communities, newsletters (channels), presence, edit/delete   | [Groups](/groups)                      |
| Storage adapters         | 🔗☁️ | `file`, `memory`, `sqlite`, `redis`, `postgres`, `convex` for auth & messages | [Storage](/storage)          |
| Cross-runtime            | 🔗☁️ | Dual ESM/CJS, full types; Node, Bun, Deno, Termux                    | [Runtimes](/runtimes)                  |
