...
| ------------- | ------------------------------------------- | ---------------------------------------- | ----------------------------------------------------------- |
| `packageName` | `string`                                    | `'Zaileys Library'`                      | Sticker pack name stored in EXIF (aliases: `pack`, `packname`). |
| `authorName`  | `string`                                    | `'https://github.com/zeative/zaileys'`   | Sticker pack publisher stored in EXIF (aliases: `author`, `publisher`). |
| `pack`        | `string`                                    | —                                        | Convenient alias for `packageName`.                         |
| `author`      | `string`                                    | —                                        | Convenient alias for `authorName`.                          |
...
### Global Client Sticker Defaults

You can configure global default sticker metadata when initializing the `Client`:

```typescript

const client = new Client({
  // Global default metadata for all stickers
  sticker: {
    pack: 'My Bot Sticker Pack',
    author: 'My Bot',
  },
})
```

### Passing Metadata in MessageBuilder

You can also pass custom pack and author metadata per-sticker directly in `.sticker()`:

```typescript
await client.send('6281234567890@s.whatsapp.net').sticker(imageBuffer, {
  pack: 'Custom Pack',
