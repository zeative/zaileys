...
| `call` | `call(value): Promise<void>` | `void` | Restrict who may call you: `all` or `known` (contacts only). |
...
| `call` | `WAPrivacyCallValue` | `'all'` \| `'known'` | Who can call you. `'known'` silences calls from anyone outside your contacts. |
...
```typescript
call(value: 'all' | 'known'): Promise<void>
```

Restricts who may call you. `'known'` silences calls from anyone who is not in your contacts —
WhatsApp does not ring, and the caller is not told why.

### Example

```typescript
// Only contacts can call
await client.privacy.call('known')

// Also reachable through set(), alongside other settings
await client.privacy.set({ call: 'known', lastSeen: 'contacts' })
```

This changes who is *allowed* to call. To hang up on calls that still come through, see
[`autoRejectCall`](/configuration) — the two work well together.
