...
13. **`groupStatus` is GROUPS-only and validates AFTER username resolution** — a non-`@g.us` recipient rejects with `INVALID_RECIPIENT` at `await`, not at call time. Reposting (`groupStatus(ctx)`) is TEXT-ONLY — media throws `INVALID_OPTIONS`: 8 live variants relayed fine but none rendered, while a media `status@broadcast` from the same session DID render, so the envelope is the limit. For media status use a personal status: `status@broadcast` + `statusJidList` of REAL recipient PNs (self-only or empty = reaches nobody, no error).
