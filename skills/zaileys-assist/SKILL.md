...
13. **`groupStatus` is GROUPS-only and validates AFTER username resolution** — a non-`@g.us` recipient rejects with `INVALID_RECIPIENT` at `await`, not at call time. Reposting (`groupStatus(ctx)`) is TEXT-ONLY — media sources throw `INVALID_OPTIONS` because WhatsApp accepts a media group status but never renders it (`getMediaType` doesn't see inside the envelope, so `mediatype` is never set).
