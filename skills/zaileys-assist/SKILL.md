...
13. **`groupStatus` is GROUPS-only and validates AFTER username resolution** — a non-`@g.us` recipient rejects with `INVALID_RECIPIENT` at `await`, not at call time. Reposting (`groupStatus(ctx)`) copies media POINTERS, so it never downloads/re-uploads — but the status breaks if WhatsApp expires the original media.
