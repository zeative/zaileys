...
| `htmlApp(markup, { title, height, device, fallback, maxBytes, bypassDownload })` | `string` or `html` tag result | Runs inside the bubble on **WhatsApp Android only**, offline: no network, storage, links, or scrolling; the card can't reach the bot. `device` other than `'android'` sends `fallback` text or throws `INVALID_RECIPIENT`. Build pages with `html` so values are escaped; quote attribute values and pass data to `<script>` only through `htmlJson` (the escaping doesn't cover script, style, or unquoted attributes). Default `maxBytes` 256 KB. `bypassDownload` defaults **on**: follows the card with an identical edit so no Download prompt appears, costing one extra relay and a reload whenever the keyboard opens — pass `false` for stateful pages (games, counters). Ready-made cards to adapt: https://zaileys.kejaa.id/templates/index | `SEND_FAILED` |
