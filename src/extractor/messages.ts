...
export const MessagesExtractor = async (client: Client & { db: JsonDBInterface }, message: proto.IWebMessageInfo, isExtract?: boolean) => {
  ...
  const extract = async (obj: proto.IWebMessageInfo, isReplied?: boolean) => {
    ...
    payload.isChannel = _.includes(payload.roomId, "@newsletter")!;
    ...
    payload.channelId = _.join([_.split(payload.roomId, "@")[0], _.split(payload.senderId, "@")[0]], "-");
    payload.uniqueId = _.join([payload.channelId, payload.chatId], "-");
      ...
      for (const key of Object.keys(citation!)) {
        ...
        if (citationEntry && Array.isArray(citationEntry)) {
          ...
          payload.citation[slug] =
            (senderId ? (citationRecord[key] || []).includes(Number(senderId)) : false) || (roomId ? (citationRecord[key] || []).includes(Number(roomId)) : false);
        }
      }
    }

    const media =
      (msg?.message?.editedMessage?.[contentType] as Record<string, unknown>) ||
      (msg?.message?.protocolMessage?.editedMessage?.[contentType] as Record<string, unknown>) ||
      (msg?.message?.[contentType!]?.message?.documentMessage as Record<string, unknown>) ||
      (msg?.message?.[contentType!] as Record<string, unknown>);
    ...
    const text = (
      typeof media == "string"
        ? media
        : media?.text || media?.caption || media?.name || media?.displayName || media?.conversation || media?.contentText || media?.selectedDisplayText || ""
    ) as string;
    ...
    payload.isPrefix = !!(client.props?.prefix && _.startsWith(payload.text!, client.props?.prefix));
    payload.isTagMe = _.includes(payload.mentions, _.split(payload.receiverId, "@")[0] || "");
